...
auto pv_acceleration(const field<Field, Real, 2, 2>& vf, linspace<Real> x,
  ...
  auto const pv_lines =
      parallel_vectors(stvf, staf, x, y, z, std::forward<Preds>(preds)...);
  //return pv_lines;
  std::vector<line<Real, 3>> filtered_pv;
  for (auto const& pv_line : pv_lines) {
    auto f = pv_line.filter([&](auto const& x, auto const /*i*/) {
      auto const eig = eigenvalues(grad_stvf(x, 0));
      //return true;
      return std::abs(eig(0).imag()) > 0 ||
             std::abs(eig(1).imag()) > 0 ||
             std::abs(eig(2).imag()) > 0;
    });
    std::copy(begin(f), end(f), std::back_inserter(filtered_pv));
  }
  return filtered_pv;
}
//------------------------------------------------------------------------------
...
auto pv_acceleration(const field<Field, Real, 3, 3>& vf, linspace<Real> x,
  ...
  auto       tvf = diff_time(vf, 1e-7);
  auto       af  = Jvf * vf;
  auto const pv_lines =
      parallel_vectors(vf, af + tvf, x, y, z, std::forward<Preds>(preds)...);
  return filter_length(pv_lines, 3);
}
//------------------------------------------------------------------------------
template <typename Field, typename Real, typename... Preds>
...
TEST_CASE("parallel_vectors_numerical_doublegyre_acceleration",
  ...
  write_vtk(
      pv_acceleration(
        analytical::fields::numerical::doublegyre{},
        linspace{1e-3, 2.0 - 1e-4, 51},
        linspace{1e-4, 1 - 1e-3, 26},
...
//==============================================================================
TEST_CASE("parallel_vectors_numerical_tornado_acceleration",
          "[parallel_vectors][pv][numerical][tornado]"
          "[differentiate][acceleration]") {
  ...
  write_vtk(
      pv_acceleration(
        v,
        linspace{-10.0 + 1e-3, 10.0 + 1e-5, 301},
        linspace{-10.0 + 1e-4, 10.0 + 1e-4, 301},
        linspace{-10.0 + 1e-5, 10.0 + 1e-3, 301},
        [&](auto const& x) {
           auto const eig = eigenvalues(J(x, 0));
           return std::abs(eig(0).imag()) > 0 ||
                  std::abs(eig(1).imag()) > 0 ||
                  std::abs(eig(2).imag()) > 0;
        }),
      "numerical_tornado_pv_lines_acceleration.vtk");
}
//==============================================================================
