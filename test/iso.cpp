...
TEST_CASE("isolines_doublegyre_length", "[iso][isolines][numerical][doublegyre]") {
  auto const l = euclidean_length(analytical::numerical::doublegyre{});
  SECTION("field") {
    isolines(l,
             rectilinear_grid{linspace{0.0, 2.0, 200}, linspace{0.0, 1.0, 100}},
             0.1);
  }
  SECTION("sampled") {
...
TEST_CASE("isosurface_random",
  ...
  auto data = dynamic_multidim_array<double>::rand(
      random::uniform{-1.0, 1.0, std::mt19937{1234}}, 20, 20, 20);
  isosurface(data,
             axis_aligned_bounding_box{vec3{-10, -10, -10}, vec3{10, 10, 10}},
             0);
}
//==============================================================================
}  // namespace tatooine::test
