...
TEST_CASE("for_loop_parallel_count", "[for_loop][parallel][count]") {
  ...
  SECTION("from 10 with c-arrays") {
    std::atomic_size_t cnt       = 0;
    auto               iteration = [&](auto const... /*is*/) { ++cnt; };
    for_loop(iteration, execution_policy::parallel, {10, 20}, {10, 20}, {10, 20});
    REQUIRE(cnt == 1000);
  }
}
//------------------------------------------------------------------------------
...
TEST_CASE("for_loop_seq", "[for_loop][sequential][count]") {
  SECTION("from zero") {
    ...
    for_loop(iteration, execution_policy::sequential, 10, 10, 10);
    REQUIRE(i == 1000);
  }
  SECTION("from 10 with c-arrays") {
    size_t i = 0;
    // indices used as parameter pack
    auto iteration = [&](auto... /*is*/) { ++i; };
    for_loop(iteration, execution_policy::sequential, {10, 20}, {10, 20}, {10, 20});
    REQUIRE(i == 1000);
  }
  SECTION("from 10 with std::pair") {
    ...
    // indices used as parameter pack
    auto iteration = [&](auto... /*is*/) { ++i; };
    for_loop(iteration, execution_policy::sequential, std::pair{10, 20}, std::pair{10, 20},
             std::pair{10, 20});
    REQUIRE(i == 1000);
  }
}
