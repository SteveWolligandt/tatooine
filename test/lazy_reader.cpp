...
//}
////==============================================================================
// TEST_CASE("lazy_reader_netcdf_grid",
// "[lazy_reader][netcdf][rectilinear_grid]") {
...
//    auto       dataset =
//        file.create_dataset<dataset_type>(dataset_name, data_size, data_size,
//        data_size);
//    auto data_arr =
//        dynamic_multidim_array<dataset_type>{chunk_size, chunk_size,
//        chunk_size};
//    auto i = size_t(0);
...
TEST_CASE("lazy_reader_hdf5", "[lazy_reader][hdf5]") {
  ...
  auto full_size   = std::array<size_t, 3>{64, 64, 64};
  using value_type = int;
  auto wdata =
      dynamic_multidim_array<value_type>::randu(-1000, 1000, full_size);
  {
  ...
  auto arr_lazy = arr_in.read_lazy({2, 8, 4});
  ...
  for (size_t i = 0; i < 3; ++i) {
    for_loop(
        [&](auto const x, auto const y, auto const z) {
          ...
          //CAPTURE(i, j, x, y, z);
          auto r = wdata[j] == arr_lazy(x, y, z);
          auto l = std::lock_guard{m};
          REQUIRE(r);
        },
        execution_policy::parallel, full_size);
  }
}
#endif
//==============================================================================
