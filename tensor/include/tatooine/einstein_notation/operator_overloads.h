...
//==============================================================================
#include <tatooine/einstein_notation/added_contracted_dynamic_tensor.h>
#include <tatooine/einstein_notation/added_contracted_static_tensor.h>
#include <tatooine/einstein_notation/contracted_dynamic_tensor.h>
#include <tatooine/einstein_notation/contracted_static_tensor.h>
...
template <typename TensorLHS, index... IndicesLHS, typename TensorRHS,
          index... IndicesRHS>
auto constexpr contract(indexed_dynamic_tensor<TensorLHS, IndicesLHS...> lhs,
                        indexed_dynamic_tensor<TensorRHS, IndicesRHS...> rhs) {
  return contracted_dynamic_tensor<
      indexed_dynamic_tensor<TensorLHS, IndicesLHS...>,
      indexed_dynamic_tensor<TensorRHS, IndicesRHS...>>{lhs, rhs};
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <typename TensorLHS, index... IndicesLHS, typename TensorRHS,
          index... IndicesRHS>
auto operator*(indexed_dynamic_tensor<TensorLHS, IndicesLHS...> lhs,
               indexed_dynamic_tensor<TensorRHS, IndicesRHS...> rhs) {
...
auto constexpr contract(indexed_static_tensor<TensorLHS, IndicesLHS...> lhs,
  ...
  return contracted_static_tensor<
      indexed_static_tensor<TensorLHS, IndicesLHS...>,
      indexed_static_tensor<TensorRHS, IndicesRHS...>>{lhs, rhs};
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <typename TensorLHS, index... IndicesLHS, typename TensorRHS,
          index... IndicesRHS>
...
template <typename... IndexedTensorLHS, typename TensorRHS, index... IndicesRHS,
          std::size_t... LHSSeq>
auto contract(contracted_static_tensor<IndexedTensorLHS...>   lhs,
              indexed_static_tensor<TensorRHS, IndicesRHS...> rhs,
              std::index_sequence<LHSSeq...> /*lhs_seq*/) {
  return contracted_static_tensor<
      IndexedTensorLHS..., indexed_static_tensor<TensorRHS, IndicesRHS...>>{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <typename... IndexedTensorLHS, typename TensorRHS, index... IndicesRHS>
auto operator*(contracted_static_tensor<IndexedTensorLHS...>   lhs,
               indexed_static_tensor<TensorRHS, IndicesRHS...> rhs) {
  return contract(lhs, rhs);
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
auto contract(indexed_static_tensor<TensorLHS, IndicesLHS...> lhs,
  ...
  return contracted_static_tensor<
      indexed_static_tensor<TensorLHS, IndicesLHS...>, IndexedTensorRHS...>{
      lhs, rhs.template at<RHSSeq>()...};
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <index... IndicesLHS, typename TensorLHS, typename... IndexedTensorRHS>
auto contract(indexed_static_tensor<TensorLHS, IndicesLHS...> lhs,
...
auto operator+(indexed_static_tensor<TensorLHS, IndicesLHS...> lhs,
               indexed_static_tensor<TensorRHS, IndicesRHS...> rhs) {
  return added_contracted_static_tensor{
      contracted_static_tensor<indexed_static_tensor<TensorLHS, IndicesLHS...>>{
          lhs},
      contracted_static_tensor<indexed_static_tensor<TensorRHS, IndicesRHS...>>{
          rhs}};
}
//------------------------------------------------------------------------------
template <typename... IndexedTensorLHS, typename TensorRHS, index... IndicesRHS>
...
auto operator+(contracted_static_tensor<TensorsLHS...> lhs,
               contracted_static_tensor<TensorsRHS...> rhs) {
  return added_contracted_static_tensor{lhs, rhs};
}
//------------------------------------------------------------------------------
template <typename... ContractedTensorsLHS, typename... TensorsRHS,
          std::size_t... Seq>
auto add(added_contracted_static_tensor<ContractedTensorsLHS...> lhs,
         contracted_static_tensor<TensorsRHS...>                 rhs,
         std::index_sequence<Seq...>) {
  return added_contracted_static_tensor{lhs.template at<Seq>()..., rhs};
}
//------------------------------------------------------------------------------
template <typename... TensorsLHS, typename... ContractedTensorsRHS,
          std::size_t... Seq>
auto add(contracted_static_tensor<TensorsLHS...>                 lhs,
         added_contracted_static_tensor<ContractedTensorsRHS...> rhs,
         std::index_sequence<Seq...>) {
  return added_contracted_static_tensor{lhs, rhs.template at<Seq>()...};
}
//------------------------------------------------------------------------------
...
auto add(added_contracted_static_tensor<ContractedTensorsLHS...> lhs,
  ...
  return added_contracted_static_tensor{lhs.template at<Seq0>()...,
...
//------------------------------------------------------------------------------
template <typename... ContractedTensorsLHS, typename... TensorsRHS>
auto operator+(added_contracted_static_tensor<ContractedTensorsLHS...> lhs,
               contracted_static_tensor<TensorsRHS...>                 rhs) {
  return add(lhs, rhs);
}
//------------------------------------------------------------------------------
