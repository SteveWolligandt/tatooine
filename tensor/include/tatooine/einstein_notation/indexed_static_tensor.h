...
struct indexed_static_tensor {
 ...
 public:
  ...
  requires(!is_const<std::remove_reference_t<Tensor>>) auto operator+=(
      contracted_tensor<IndexedTensors...> other) -> indexed_static_tensor& {
  ...
  requires(!is_const<std::remove_reference_t<Tensor>>) auto operator=(
      indexed_static_tensor<Tensors, Is...> other) -> indexed_static_tensor& {
    m_tensor = tensor_type::zeros();
  ...
  auto operator=(contracted_tensor<indexed_static_tensor<LHS, I, J>,
