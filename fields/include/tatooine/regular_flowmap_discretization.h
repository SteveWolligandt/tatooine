  ...
  auto fill(Flowmap&& flowmap, ExecutionPolicy execution_policy) -> void {
    m_forward_grid.sample_to_vertex_property(
        [&](auto const& x) mutable {
          ...
          auto const map = flowmap(x, m_t0, m_tau);
          return map;
        },
    ...
    m_forward_grid.vertices().iterate_indices(
        [&, i = std::atomic_size_t{}](auto const... is) mutable {
          auto const idx = i++;
          m_backward_grid.vertex_at(idx) =
              m_forward_flowmap_discretization->at(is...);
          m_backward_flowmap_discretization->at(idx) =
              m_forward_grid.vertex_at(is...);
        },
        execution_policy);
    m_backward_sampler =
...
//   //----------------------------------------------------------------------------
//   /// \{
//   auto flowmap(forward_tag const /*tag*/) const -> auto const& {
//     return *m_forward_flowmap_discretization;
//   }
//   //----------------------------------------------------------------------------
//   auto flowmap(backward_tag const /*tag*/) -> auto& {
//     return *m_backward_flowmap_discretization;
//   }
//   /// \}
//   //----------------------------------------------------------------------------
