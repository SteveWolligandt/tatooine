#ifndef TATOOINE_ODE_BOOST_CONTROLLED_RUNGE_KUTTA_WITH_DOMAIN_CHECK_H
#define TATOOINE_ODE_BOOST_CONTROLLED_RUNGE_KUTTA_WITH_DOMAIN_CHECK_H
//==============================================================================
...
//==============================================================================
template <typename Value, typename Algebra = ::boost::numeric::odeint::range_algebra,
          typename Operations = ::boost::numeric::odeint::default_operations>
class default_error_checker {
 public:
  ...
  value_type error(algebra_type &algebra, const State &x_old,
    ...
    algebra.for_each3(
        x_err, x_old, dxdt_old,
        typename operations_type::template rel_error<value_type>(
            m_eps_abs, m_eps_rel, m_a_x, m_a_dxdt * get_unit_value(dt)));

    value_type res = algebra.reduce(
        x_err, typename operations_type::template maximum<value_type>(),
...
template <typename ErrorStepper,
          typename ErrorChecker =
              default_error_checker<typename ErrorStepper::value_type,
 ...
 *
 * \tparam ErrorStepper The stepper type with error estimation, has to fulfill
 * the ErrorStepper concept. \tparam ErrorChecker The error checker \tparam
 * Resizer The resizer policy type.
 */
template <typename ErrorStepper, typename ErrorChecker, typename Resizer>
class controlled_runge_kutta_with_domain_check<
    ErrorStepper, ErrorChecker, Resizer,
    ::boost::numeric::odeint::explicit_error_stepper_tag> {
 public:
  using stepper_type       = ErrorStepper;
  ...
  using resizer_type       = Resizer;
  using error_checker_type = ErrorChecker;
  using stepper_category   = explicit_controlled_stepper_tag;
  ...
  using controlled_stepper_type =
      controlled_runge_kutta_with_domain_check<ErrorStepper, ErrorChecker, Resizer,
   ...
   * performed.
   *
   * \param system The system function to solve, hence the r.h.s. of the ODE. It
   * must fulfill the Simple System concept. \param x The state of the ODE which
   * should be solved. Overwritten if the step is successful. \param t The value
   * of the time. Updated if the step is successful. \param dt The step size.
   * Updated. \return success if the step was accepted, fail otherwise.
   */
  template <typename System, typename StateInOut>
  controlled_step_result try_step(System system, StateInOut &x, time_type &t,
                                  time_type &dt) {
   ...
   * \param system The system function to solve, hence the r.h.s. of the ODE. It
   * must fulfill the Simple System concept. \param x The state of the ODE which
   * should be solved. Overwritten if the step is successful. Can be a boost
   * range. \param t The value of the time. Updated if the step is successful.
   * \param dt The step size. Updated.
   * \return success if the step was accepted, fail otherwise.
   */
  template <typename System, typename StateInOut>
  controlled_step_result try_step(System system, const StateInOut &x,
   ...
   *
   * this version does not solve the forwarding problem, boost.range can not be
   * used
   */
  /**
   * \brief Tries to perform one step.
   ...
   * \param system The system function to solve, hence the r.h.s. of the ODE. It
   * must fulfill the Simple System concept. \param x The state of the ODE which
   * should be solved. Overwritten if the step is successful. \param dxdt The
   * derivative of state. \param t The value of the time. Updated if the step is
   * successful. \param dt The step size. Updated. \return success if the step
   * was accepted, fail otherwise.
   */
  template <typename System, typename StateInOut, typename DerivIn>
  controlled_step_result try_step(System system, StateInOut &x,
                                  const DerivIn &dxdt, time_type &t,
                                  time_type &dt) {
    m_xnew_resizer.adjust_size(
        x, detail::bind(
               &controlled_runge_kutta_with_domain_check::template resize_m_xnew_impl<StateInOut>,
   ...
   * used
   *
   * the disable is needed to avoid ambiguous overloads if state_type =
   * time_type
   */
  /**
   * \brief Tries to perform one step.
   ...
   *
   * \param system The system function to solve, hence the r.h.s. of the ODE. It
   * must fulfill the Simple System concept. \param in The state of the ODE
   * which should be solved. \param t The value of the time. Updated if the step
   * is successful. \param out Used to store the result of the step. \param dt
   * The step size. Updated. \return success if the step was accepted, fail
   * otherwise.
   */
  template <typename System, typename StateIn, typename StateOut>
  typename boost::disable_if<boost::is_same<StateIn, time_type>,
                             controlled_step_result>::type
  try_step(System system, const StateIn &in, time_type &t, StateOut &out,
           time_type &dt) {
    typename odeint::unwrap_reference<System>::type &sys = system;
    m_dxdt_resizer.adjust_size(
        in, detail::bind(
                &controlled_runge_kutta_with_domain_check::template resize_m_dxdt_impl<StateIn>,
                detail::ref(*this), detail::_1));
    sys(in, m_dxdt.m_v, t);
   ...
   * \param system The system function to solve, hence the r.h.s. of the ODE. It
   * must fulfill the Simple System concept. \param in The state of the ODE
   * which should be solved. \param dxdt The derivative of state. \param t The
   * value of the time. Updated if the step is successful. \param out Used to
   * store the result of the step. \param dt The step size. Updated. \return
  ...
  controlled_step_result try_step(System system, const StateIn &in,
                                  const DerivIn &dxdt, time_type &t,
    ...
    m_xerr_resizer.adjust_size(
        in, detail::bind(
                &controlled_runge_kutta_with_domain_check::template resize_m_xerr_impl<StateIn>,
                detail::ref(*this), detail::_1));
    ...
    m_max_rel_error =
        m_error_checker.error(m_stepper.algebra(), in, dxdt, m_xerr.m_v, dt);

    if (m_max_rel_error > 1.0) {
      // error too large - decrease dt ,limit scaling factor to 0.2 and reset
      // state
      dt *= max BOOST_PREVENT_MACRO_SUBSTITUTION(
          static_cast<value_type>(9) / static_cast<value_type>(10) *
              pow(m_max_rel_error,
      ...
      if (m_max_rel_error < 0.5) {
        ...
        m_max_rel_error = max BOOST_PREVENT_MACRO_SUBSTITUTION(
            pow(5.0, -m_stepper.stepper_order()), m_max_rel_error);
        // error too small - increase dt and keep the evolution and limit
        // scaling factor to 5.0
        t += dt;
   ...
   */
  value_type last_error(void) const { return m_max_rel_error; }

  /**
   * \brief Adjust the size of all temporaries in the stepper manually.
   * \param x A state from which the size of the temporaries to be resized is
   * deduced.
   */
 ...
 private:
  template <typename System, typename StateInOut>
  controlled_step_result try_step_v1(System system, StateInOut &x, time_type &t,
                                     time_type &dt) {
    typename odeint::unwrap_reference<System>::type &sys = system;
    m_dxdt_resizer.adjust_size(
        x, detail::bind(
               &controlled_runge_kutta_with_domain_check::template resize_m_dxdt_impl<StateInOut>,
               detail::ref(*this), detail::_1));
    sys(x, m_dxdt.m_v, t);
  ...
  bool resize_m_dxdt_impl(const StateIn &x) {
    return adjust_size_by_resizeability(
        m_dxdt, x, typename is_resizeable<deriv_type>::type());
  }
 ...
 */
template <typename ErrorStepper, typename ErrorChecker, typename Resizer>
class controlled_runge_kutta_with_domain_check<ErrorStepper, ErrorChecker, Resizer,
 ...
 public:
  ...
  using resizer_type       = Resizer;
  using error_checker_type = ErrorChecker;
  using stepper_category   = explicit_controlled_stepper_fsal_tag;
  ...
  controlled_step_result try_step(System system, StateInOut &x,
                                  DerivInOut &dxdt, time_type &t,
                                  time_type &dt) {
    m_xnew_resizer.adjust_size(
        x, detail::bind(
               &controlled_runge_kutta_with_domain_check::template resize_m_xnew_impl<StateInOut>,
               detail::ref(*this), detail::_1));
    m_dxdt_new_resizer.adjust_size(
        x,
        detail::bind(&controlled_runge_kutta_with_domain_check::template resize_m_dxdt_new_impl<
                         StateInOut>,
                     detail::ref(*this), detail::_1));
    controlled_step_result res =
        try_step(system, x, dxdt, t, m_xnew.m_v, m_dxdtnew.m_v, dt);
    if (res == success) {
      boost::numeric::odeint::copy(m_xnew.m_v, x);
  ...
  template <typename System, typename StateIn, typename DerivIn, typename StateOut,
            typename DerivOut>
  controlled_step_result try_step(System system, const StateIn &in,
                                  const DerivIn &dxdt_in, time_type &t,
                                  StateOut &out, DerivOut &dxdt_out,
                                  time_type &dt) {
    BOOST_USING_STD_MIN();
    BOOST_USING_STD_MAX();
    ...
    // this potentially overwrites m_x_err! (standard_error_checker does, at
    // least)
    value_type max_rel_err =
        m_error_checker.error(m_stepper.algebra(), in, dxdt_in, m_xerr.m_v, dt);
    ...
    if (max_rel_err > 1.0) {
      ...
      dt *= max BOOST_PREVENT_MACRO_SUBSTITUTION(
          static_cast<value_type>(
              ...
              pow(max_rel_err,
                  static_cast<value_type>(-1) / (m_stepper.error_order() - 1))),
          static_cast<value_type>(static_cast<value_type>(1) /
                                  static_cast<value_type>(5)));
      return fail;
    } else {
      if (max_rel_err < 0.5) {  // error too small - increase dt and keep the
                                // evolution and limit scaling factor to 5.0
        // error should be > 0
        max_rel_err = max BOOST_PREVENT_MACRO_SUBSTITUTION(
            pow(5.0, -m_stepper.stepper_order()), max_rel_err);
        t += dt;
        dt *= static_cast<value_type>(
            static_cast<value_type>(9) / static_cast<value_type>(10) *
            pow(max_rel_err,
                static_cast<value_type>(-1) / m_stepper.stepper_order()));
        return success;
   ...
   */
  void reset(void) { m_first_call = true; }

  /**
   * \brief Initializes the internal state storing an internal copy of the
   * derivative.
   *
   ...
   *
   * \param system The system function to solve, hence the r.h.s. of the ODE. It
   * must fulfill the Simple System concept. \param x The initial state of the
   * ODE which should be solved. \param t The initial time.
   */
   ...
   */
  bool is_initialized(void) const { return !m_first_call; }
 ...
 private:
  ...
  bool resize_m_dxdt_new_impl(const StateIn &x) {
    return adjust_size_by_resizeability(
        m_dxdtnew, x, typename is_resizeable<deriv_type>::type());
  }
  ...
  controlled_step_result try_step_v1(System system, StateInOut &x, time_type &t,
    ...
    if (m_dxdt_resizer.adjust_size(
            x,
            detail::bind(&controlled_runge_kutta_with_domain_check::template resize_m_dxdt_impl<
                             StateInOut>,
                         detail::ref(*this), detail::_1)) ||
        m_first_call) {
 ...
 * It is used by the controlled_runge_kutta_with_domain_check steppers.
 *
...
/**
 * \fn default_error_checker( value_type eps_abs , value_type eps_rel ,
 * value_type a_x , value_type a_dxdt ) \brief Constructs the error checker.
 *
...
/**
 * \fn error( const State &x_old , const Deriv &dxdt_old , Err &x_err , Time dt
 * ) const \brief Calculates the error level.
 *
...
/**
 * \fn error( algebra_type &algebra , const State &x_old , const Deriv &dxdt_old
 * , Err &x_err , Time dt ) const \brief Calculates the error level using a
 * given algebra.
 *
 * If the returned error level is greater than 1, the estimated error was
 * larger than the permitted error bounds and the step should be repeated
