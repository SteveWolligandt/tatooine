...
//==============================================================================
template <typename Real, size_t N, typename Prop>
struct line_vertex_property_sampler<Real, N, Prop, interpolation::linear> {
 ...
 public:
  ...
  auto operator()(Real t) const {
    ...
    //std::cerr << "--------\n";
    //std::cerr << lt << ' ' << rt << ' ' << t << '\n';
    //std::cerr << m_prop[range.first] << ' ' <<  m_prop[range.second] << '\n';
    //std::cerr << interpolation::linear{m_prop[range.first], m_prop[range.second]}(t) << '\n';

    return interpolation::linear{m_prop[range.first], m_prop[range.second]}(t);
  }
};
template <typename Real, size_t N, typename Prop>
struct line_vertex_property_sampler<Real, N, Prop, interpolation::cubic> {
  using line_t             = line<Real, N>;
  using handle_t           = typename line<Real, N>::vertex_handle;
  using tangent_property_t = typename line_t::tangent_property_t;
  using parameterization_property_t =
      typename line_t::parameterization_property_t;
  using value_type =
      std::decay_t<decltype(std::declval<Prop>()[std::declval<handle_t>()])>;
 ...
 public:
  line_vertex_property_sampler(line_t const& line, Prop const& prop)
    ...
    auto dfdt0 = derivative(m_line.vertices().front());
    for (auto it = m_line.vertices().begin();
         it != prev(m_line.vertices().end()); ++it) {
      auto       v0    = *it;
      auto       v1    = *next(it);
      auto const dfdt1 = derivative(v1);
      auto const dy    = m_parameterization[v1] - m_parameterization[v0];
      m_interpolants.emplace_back(m_prop[v0], m_prop[v1], dfdt0 * dy,
                                  dfdt1 * dy);
      dfdt0 = dfdt1;
    }
  }
