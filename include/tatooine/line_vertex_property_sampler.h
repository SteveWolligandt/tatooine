 ...
 public:
  ...
  auto operator()(Real t) const {
    ...
    while (range.second.index() - range.first.index() > 1) {
      auto const center =
          handle_t{(range.first.index() + range.second.index()) / 2};
 ...
 public:
  line_vertex_property_sampler(line_t const& line, Prop const& prop)
    ...
    auto const derivative = [&](auto const v) {
      auto       lv = half > v.index() ? handle_t{0} : v - half;
      auto const rv = lv.index() + stencil_size - 1 >= m_line.num_vertices()
                          ? handle_t{m_line.num_vertices() - 1}
                          : lv + stencil_size - 1;
      auto const rpotential = stencil_size - (rv.index() - lv.index() + 1);
      lv = rpotential > lv.index() ? handle_t{0} : lv - rpotential;
