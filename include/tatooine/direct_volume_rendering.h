...
auto direct_volume_rendering(
  ...
  auto& rendering =
      rendered_image.template add_vertex_property<color_type>(
          "rendering");
...
auto direct_volume_rendering(
  ...
  return direct_volume_rendering(
      cam, bb, [&](auto const& x) { return s(x, t); },
...
auto direct_volume_rendering(
    ...
    std::invoke_result_t<ColorScale, typename Container::value_type> const&
        bg_color = {}) {
  return direct_volume_rendering(
      cam, prop.grid().axis_aligned_bounding_box(),
      [&](auto const& x) { return prop.sample(x); },
