...
struct numerically_differentiated_flowmap {
 ...
 public:
  ...
  static constexpr auto default_epsilon = 1e-8;
  //============================================================================
 private:
...
numerically_differentiated_flowmap(Flowmap const&,
    ...
    -> numerically_differentiated_flowmap<Flowmap>;
//==============================================================================
auto diff(flowmap_concept auto const&& flowmap, tag::numerical_t /*tag*/) {
  return numerically_differentiated_flowmap{flowmap};
}
//------------------------------------------------------------------------------
auto diff(flowmap_concept auto const&& flowmap, tag::numerical_t /*tag*/,
          arithmetic auto const        epsilon) {
  return numerically_differentiated_flowmap{flowmap, epsilon};
}
//------------------------------------------------------------------------------
auto diff(
    flowmap_concept auto const&& flowmap, tag::numerical_t /*tag*/,
    fixed_size_real_vec<
        std::decay_t<decltype(flowmap)>::num_dimensions()> auto&& epsilon) {
  return numerically_differentiated_flowmap{
      flowmap, std::forward<decltype(epsilon)>(epsilon)};
}
//------------------------------------------------------------------------------
auto diff(flowmap_concept auto const&& flowmap, arithmetic auto const epsilon) {
  return diff(flowmap, tag::numerical, epsilon);
}
//------------------------------------------------------------------------------
auto diff(
    flowmap_concept auto const&& flowmap,
    fixed_size_real_vec<
        std::decay_t<decltype(flowmap)>::num_dimensions()> auto&& epsilon) {
  return numerically_differentiated_flowmap{
      flowmap, std::forward<decltype(epsilon)>(epsilon)};
...
auto diff(
    ...
    fixed_size_real_vec<
        std::decay_t<decltype(flowmap)>::num_dimensions()> auto&& epsilon) {
  return diff(flowmap, tag::numerical, std::forward<decltype(epsilon)>(epsilon));
}
//==============================================================================
...
differentiated_flowmap(Flowmap const&)
    -> differentiated_flowmap<Flowmap const&>;
//==============================================================================
auto diff(flowmap_concept auto const&& flowmap) {
