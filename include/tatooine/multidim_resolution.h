...
#endif
//==============================================================================
struct dynamic_multidim_iterator;
//==============================================================================
struct dynamic_multidim {
 ...
 public:
  //----------------------------------------------------------------------------
  constexpr dynamic_multidim(const std::vector<std::pair<size_t, size_t>>& ranges)
      : m_ranges(ranges) {}
  //----------------------------------------------------------------------------
  constexpr dynamic_multidim(std::vector<std::pair<size_t, size_t>>&& ranges)
      : m_ranges(std::move(ranges)) {}
  //----------------------------------------------------------------------------
  constexpr dynamic_multidim(const std::vector<size_t>& res)
      : m_ranges(res.size(), std::make_pair<size_t, size_t>(0, 0)) {
    for (size_t i = 0; i < res.size(); ++i) { m_ranges[i].second = res[i]; }
  }
  //----------------------------------------------------------------------------
  template <size_t N>
  constexpr dynamic_multidim(const std::array<std::pair<size_t, size_t>, N>& ranges)
      : m_ranges(begin(ranges), end(ranges)) {}
  //----------------------------------------------------------------------------
  template <size_t N>
  ...
  //----------------------------------------------------------------------------
  constexpr auto begin() {
    return dynamic_multidim_iterator<N>{*this, make_array<size_t, N>()};
  }
  //----------------------------------------------------------------------------
  constexpr auto end() {
    auto a   = make_array<size_t, N> a();
    a.back() = m_ranges.back().second;
    return dynamic_multidim_iterator<N>{*this, std::move(a)};
  }
};
...
struct dynamic_multidim_iterator {
  //----------------------------------------------------------------------------
  const dynamic_multidim<N>  m_cont;
  std::array<size_t, N> m_status;

  //----------------------------------------------------------------------------
  constexpr dynamic_multidim_iterator(const dynamic_multidim<N>&        c,
                                 const std::array<size_t, N>& status)
      : m_cont{c}, m_status{status} {}

  //----------------------------------------------------------------------------
  constexpr dynamic_multidim_iterator(const dynamic_multidim_iterator& other)
      : m_cont{other.m_cont}, m_status{other.m_status} {}
  ...
  //----------------------------------------------------------------------------
  constexpr auto operator==(const dynamic_multidim_iterator& other) const {
    for (size_t i = 0; i < N; ++i) {
      if (m_status[i] != other.m_status[i]) { return false; }
    }
    return true;
...
struct static_multidim_resolution {
  ...
  static constexpr auto indices() {
    return static_multidim{{size_t(0), Resolution}...};
  }
};
//==============================================================================
