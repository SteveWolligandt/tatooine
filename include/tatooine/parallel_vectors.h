...
auto calc_parallel_vectors(GetV&& getv, GetW&& getw,
  ...
  // turned inner tet order:
  //   [0]: 0,5,3 | [1]: 0,3,6 | [2]: 3,5,6 | [3]: 0,6,5
  // non-turned inner tet order:
  //   [0]: 1,4,2 | [1]: 1,7,2 | [2]: 1,4,7 | [3]: 2,7,4
  auto inner_faces =
  ...
  // std::mutex mutex;
  //
  // auto check_faces_x = [&g, &getv, &getw, &mutex,
  // &preds...](
  ...
  // std::mutex mutex;
  //
  // auto check_faces_y = [&g, &getv, &getw, &mutex,
  // &preds...](
  //                      size_t ix, size_t iy, size_t iz) {
  //#else
   auto check_faces_y = [&g, &getv, &getw, &y_faces, &preds...](
                           size_t const iy, size_t const ix, size_t const iz)
                           {
    //#endif
    auto const gv = g.vertices();
    auto const p  = std::array{
   ...
   // std::mutex mutex;
   //
   // auto check_faces_z = [&g, &getv, &getw, &mutex,
   // &preds...](
   //                      size_t ix, size_t iy, size_t iz) {
   //#else
   auto check_faces_z = [&g, &getv, &getw, &z_faces, &preds...](
                            size_t const iz, size_t const ix, size_t const iy) {
     //#endif
     auto const gv = g.vertices();
     auto const p  = std::array{
         gv(ix, iy, iz),         // 0
   ...
   // std::mutex mutex;
   //
   // auto check_inner_faces = [&g, &getv, &getw, &mutex,
   // &preds...](
   //                      size_t ix, size_t iy, size_t iz) {
   //#else
   ...
   #if defined(NDEBUG) && defined(TATOOINE_OPENMP_AVAILABLE)
   auto constexpr exec = execution_policy::parallel;
   #else
   ...
   for_loop(
       [&](auto const ix, auto const iy, auto const iz) {
         if (turned(ix, iy, iz)) {
           // 0, 2, 3, 6
           check_tet(x_faces(ix, iy, iz)[1],      // 0,2,6
                     y_faces(ix, iy + 1, iz)[0],  // 2,3,6
                     z_faces(ix, iy, iz)[1],      // 0,2,3
                     inner_faces(ix, iy, iz)[1],  // 0,3,6
                     lines);
           // 0,1,3,5
           check_tet(x_faces(ix + 1, iy, iz)[0],  // 1,3,5
                     y_faces(ix, iy, iz)[0],      // 0,1,5
                     z_faces(ix, iy, iz)[0],      // 0,1,3
                     inner_faces(ix, iy, iz)[0],  // 0,3,5
                     lines);
           // 3,5,6,7
           check_tet(x_faces(ix + 1, iy, iz)[1],  // 3,5,7
                     y_faces(ix, iy + 1, iz)[1],  // 3,6,7
                     z_faces(ix, iy, iz + 1)[1],  // 5,6,7
                     inner_faces(ix, iy, iz)[2],  // 3,5,6
                     lines);
           // 0,4,5,6
           check_tet(x_faces(ix, iy, iz)[0],      // 0,4,6
                     y_faces(ix, iy, iz)[0],      // 0,4,5
                     z_faces(ix, iy, iz + 1)[0],  // 4,5,6
                     inner_faces(ix, iy, iz)[3],  // 0,5,6
                     lines);
           // 0,3,5,6
           check_tet(inner_faces(ix, iy, iz)[0],  // 0,3,5
                     inner_faces(ix, iy, iz)[1],  // 0,3,6
                     inner_faces(ix, iy, iz)[2],  // 0,5,6
                     inner_faces(ix, iy, iz)[3],  // 3,5,6
                     lines);
         } else {
           // 0,1,2,4
           check_tet(x_faces(ix, iy, iz)[0],      // 0,2,4
                     y_faces(ix, iy, iz)[0],      // 0,1,4
                     z_faces(ix, iy, iz)[0],      // 0,1,2
                     inner_faces(ix, iy, iz)[0],  // 1,2,4
                     lines);
           // 1,4,5,7
           check_tet(x_faces(ix + 1, iy, iz)[1],  // 1,5,7
                     y_faces(ix, iy, iz)[1],      // 1,4,5
                     z_faces(ix, iy, iz + 1)[0],  // 4,5,7
                     inner_faces(ix, iy, iz)[2],  // 1,4,7
                     lines);
           // 2,4,6,7
           check_tet(x_faces(ix, iy, iz)[1],      // 2,4,6
                     y_faces(ix, iy + 1, iz)[0],  // 2,6,7
                     z_faces(ix, iy, iz)[1],      // 4,6,7
                     inner_faces(ix, iy, iz)[3],  // 2,4,7
                     lines);
           // 1,2,3,7
           check_tet(x_faces(ix + 1, iy, iz)[0],  // 1,3,7
                     y_faces(ix, iy + 1, iz)[0],  // 2,3,7
                     z_faces(ix, iy, iz)[1],      // 1,2,3
                     inner_faces(ix, iy, iz)[1],  // 1,2,7
                     lines);
           // 1,2,4,7
           check_tet(inner_faces(ix, iy, iz)[0],  // 1,4,2
                     inner_faces(ix, iy, iz)[1],  // 1,7,2
                     inner_faces(ix, iy, iz)[2],  // 1,4,7
                     inner_faces(ix, iy, iz)[3],  // 2,7,4
                     lines);
         }
       },
       exec, g.template size<0>() - 1, g.template size<1>() - 1,
       g.template size<2>() - 1);
   return lines;
}
//==============================================================================
