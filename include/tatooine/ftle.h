...
struct ftle
    : field<ftle<V, Integrator>, typename V::real_t, V::num_dimensions()> {
  ...
  using vec_t        = vec<real_t, V::num_dimensions()>;
  ...
  public:
   ...
   tensor_t evaluate(const pos_t& x, real_t t) const {
     const auto   g       = m_flowmap_gradient(x, t);
     const auto   eigvals = eigenvalues_sym(transpose(g) * g);
     const auto   max_eig = max(eigvals);
  ...
  //----------------------------------------------------------------------------
  auto  eps() const { return m_flowmap_gradient.eps(); }
  auto& eps() { return m_flowmap_gradient.eps(); }
  auto  eps(size_t i) const {
    return m_flowmap_gradient.eps(i);
  }
  auto& eps(size_t i) { return m_flowmap_gradient.eps(i); }
  void  set_eps(real_t eps) { m_flowmap_gradient.set_eps(eps); }
  void  set_eps(const vec_t& eps) { m_flowmap_gradient.set_eps(eps); }
  void  set_eps(vec_t&& eps) { m_flowmap_gradient.set_eps(std::move(eps)); }
};
