...
struct radial_basis_functions_sampler_with_gradients
  ...
  static constexpr auto kernel_from_squared_y1_x2(Real const x1, Real const x2,
    ...
    return -((4 * x2 - 4 * x1) * y2 + (4 * x1 - 4 * x2) * y1) /
           (y2 * y2 - 2 * y1 * y2 + y1 * y1 + x2 * x2 - 2 * x1 * x2 + x1 * x1);
                                                  ...
                                                  //          x1 * x1);
                                                  //}
  static constexpr auto kernel_from_squared_y1_y2(Real const x1, Real const x2,
                                                  Real const y1,
                                                  Real const y2) {
    return -((2 * y2 * y2 - 4 * y1 * y2 + 2 * y1 * y1 + 2 * x2 * x2 -
              4 * x1 * x2 + 2 * x1 * x1) *
                 gcem::log(y2 * y2 - 2 * y1 * y2 + y1 * y1 + x2 * x2 -
                           2 * x1 * x2 + x1 * x1) +
             7 * y2 * y2 - 14 * y1 * y2 + 7 * y1 * y1 + 3 * x2 * x2 -
             6 * x1 * x2 + 3 * x1 * x1) /
           (y2 * y2 - 2 * y1 * y2 + y1 * y1 + x2 * x2 - 2 * x1 * x2 + x1 * x1);
  }
 ...
 public:
  ...
  radial_basis_functions_sampler_with_gradients(
      ...
      df_property_type const& df, execution_policy::sequential_t /*pol*/)
      : m_pointset{ps}, m_f{f}, m_df{df} {
    auto const N = m_pointset.vertices().size();
    // construct lower part of
    // symmetric matrix A
    auto A = tensor<real_type>::zeros(N * 3, N * 3);
    ...
    for (std::size_t c = 0; c < N; ++c) {
      for (std::size_t r = N; r < N * 2; ++r) {
        auto const& p1 = m_pointset.vertex_at(c);
        auto const& p2 = m_pointset.vertex_at(r);
        A(r, c)        = kernel_from_squared_x1(p1.x(), p1.y(), p2.x(), p2.y());
      }
    }
    ...
    for (std::size_t c = 0; c < N; ++c) {
      for (std::size_t r = N * 2; r < N * 3; ++r) {
        auto const& p1 = m_pointset.vertex_at(c);
        auto const& p2 = m_pointset.vertex_at(r);
        A(r, c)        = kernel_from_squared_y1(p1.x(), p1.y(), p2.x(), p2.y());
      }
    }
    ...
    for (std::size_t c = N; c < N * 2; ++c) {
      for (std::size_t r = c + 1; r < N * 2; ++r) {
        auto const& p1 = m_pointset.vertex_at(c);
        auto const& p2 = m_pointset.vertex_at(r);
        A(r, c) = -kernel_from_squared_x1_x2(p1.x(), p1.y(), p2.x(), p2.y());
      }
    }
    ...
    for (std::size_t c = N; c < N * 2; ++c) {
      for (std::size_t r = N * 2; r < N * 3; ++r) {
        auto const& p1 = m_pointset.vertex_at(c);
        auto const& p2 = m_pointset.vertex_at(r);
        A(r, c) = kernel_from_squared_y1_x2(p1.x(), p1.y(), p2.x(), p2.y());
      }
    }
    ...
    for (std::size_t c = N * 2; c < N * 3; ++c) {
      for (std::size_t r = c + 1; r < N * 3; ++r) {
        auto const& p1 = m_pointset.vertex_at(c);
        auto const& p2 = m_pointset.vertex_at(r);
        A(r, c) = kernel_from_squared_y1_y2(p1.x(), p1.y(), p2.x(), p2.y());
      }
    }

    m_coefficients = [N] {
      if constexpr (arithmetic<ValueType>) {
        return tensor<ValueType>::zeros(N * 3);
        //} else if constexpr
        //(static_tensor<ValueType>)
        //{
        //  return
  ...
  [[nodiscard]] auto evaluate(pos_type const& q, real_type const /*t*/) const
    ...
    for (auto const v : m_pointset.vertices()) {
      ...
      if constexpr (arithmetic<ValueType>) {
        acc += m_coefficients(v.index()) * kernel_from_squared(sqr_dist);
      } else if constexpr (static_tensor<ValueType>) {
        for (std::size_t j = 0; j < ValueType::num_components(); ++j) {
          acc.data()[j] +=
              m_coefficients(v.index(), j) * kernel_from_squared(sqr_dist);
        }
      }
    }
    return acc;
