...
auto main(int argc, char** argv) -> int {
  ...
  // local_grid covers all grid vertices exactly once distributed over all processes
  ...
  //write_vtk(vortex_core_lines, "pv_stdg_mpi_rank_" + std::to_string(mpi.rank()) + ".vtk");
  //------------------------------------------------------------------------------
  auto calc_J_at = [&](auto ix, auto iy, auto iz) {
    ...
    // calc x
    if (ix == 0) {
      auto const stencil = full_domain_grid.diff_stencil_coefficients_0_p1_p2(0, ix);
      J.col(0) = tat::vec3 {
        xvel_150(ix    , iy, iz) * stencil[0] + 
      ...
      auto const stencil = full_domain_grid.diff_stencil_coefficients_n2_n1_0(0, ix);
      J.col(0) = tat::vec3 {
        xvel_150(ix - 2, iy, iz) * stencil[0] + 
        xvel_150(ix - 1, iy, iz) * stencil[1] + 
      ...
      auto const stencil = full_domain_grid.diff_stencil_coefficients_n1_0_p1(0, ix);
      J.col(0) = tat::vec3 {
        xvel_150(ix - 1, iy, iz) * stencil[0] + 
    ...
    // calc y
    if (iy == 0) {
      auto const stencil =  full_domain_grid.diff_stencil_coefficients_0_p1_p2(1, iy);
      J.col(1) = tat::vec3 {
        xvel_150(ix, iy    , iz) * stencil[0] + 
      ...
      auto const stencil =  full_domain_grid.diff_stencil_coefficients_n2_n1_0(1, iy);
      J.col(1) = tat::vec3 {
      ...
      auto const stencil = full_domain_grid.diff_stencil_coefficients_n1_0_p1(1, iy);
      J.col(1) = tat::vec3 {
    ...
    // calc z
    if (iz == 0) {
      auto const stencil = full_domain_grid.diff_stencil_coefficients_0_p1_p2(2, iz);
      J.col(2) = tat::vec3 {
        xvel_150(ix, iy, iz    ) * stencil[0] + 
      ...
      auto const stencil =  full_domain_grid.diff_stencil_coefficients_n2_n1_0(2, iz);
      J.col(2) = tat::vec3 {
      ...
      auto const stencil = full_domain_grid.diff_stencil_coefficients_n1_0_p1(2, iz);
      J.col(2) = tat::vec3 {
  ...
  auto const vortex_core_lines = tat::detail::calc_parallel_vectors<double>(
    [&](auto ix, auto iy, auto iz, auto const& /*p*/) {
      ...
      iy += mpi.process_begin(1);
      iz += mpi.process_begin(2);
      return tat::vec3{xvel_150(ix, iy, iz),
                       yvel_150(ix, iy, iz),
                       zvel_150(ix, iy, iz)};
    },
    [&](auto ix, auto iy, auto iz, auto const& /*p*/) {
      ...
      iy += mpi.process_begin(1);
      iz += mpi.process_begin(2);
      tat::vec3 vel{xvel_150(ix, iy, iz),
                    yvel_150(ix, iy, iz),
                    zvel_150(ix, iy, iz)};
      return J * vel;
    },
  ...
  write_vtk(vortex_core_lines, "vortex_core_lines_channelflow_150000_mpi_rank_" + std::to_string(mpi.rank()) + ".vtk");
}
