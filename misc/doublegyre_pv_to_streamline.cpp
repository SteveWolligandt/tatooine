...
auto read_pv_lines(const std::string& filepath) {
  ...
  for (auto& l_ : lines) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      write_step(v, t0, l, c2s,
                 name + "/" + name + "_progression_" + std::to_string(cnt) + "__" +
                     std::to_string(i) + ".vtk");
      l = std::move(new_l);
...
int main() {
  ...
  progress(v, read_pv_lines(filepath_acceleration), "doublegyre_pv_acc_to_streamline");
  progress(v, read_pv_lines(filepath_jerk), "doublegyre_pv_jerk_to_streamline");
}
