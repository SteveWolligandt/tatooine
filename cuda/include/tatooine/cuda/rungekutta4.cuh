...
__device__ vec_t<Real, 2> rungekutta4_step(
  ...
  if (x.x < v.min().x || x.y > v.max().x ||
      x.y < v.min().y || x.y > v.max().y ||
        t < v.min_t() ||   t > v.max_t()) {
    return make_vec<Real>(0.0 / 0.0, 0.0 / 0.0);
  }
  ...
  const auto x2 = pos + k1 / 2;
  if (x2.x < v.min().x || x2.y > v.max().x ||
      x2.y < v.min().y || x2.y > v.max().y) {
    return make_vec<Real>(0.0 / 0.0, 0.0 / 0.0);
  }
  const auto k2 = stepwidth * v(x2, t / 2);

  const auto x3 = pos + k2 / 2;
  if (x3.x < v.min().x || x3.y > v.max().x ||
      x3.y < v.min().y || x3.y > v.max().y) {
    return make_vec<Real>(0.0 / 0.0, 0.0 / 0.0);
  }
  const auto k3 = stepwidth * v(x3, t / 2);

  const auto x4 = pos + k3;
  if (x4.x < v.min().x || x4.y > v.max().x ||
      x4.y < v.min().y || x4.y > v.max().y) {
    return make_vec<Real>(0.0 / 0.0, 0.0 / 0.0);
  }
  const auto k4      = stepwidth * v(x4, t);

  const auto stepped = pos + (k1 + 2 * k2 + 2 * k3 + k4) / 6;
  if (stepped.x < v.min().x || stepped.y > v.max().x ||
