#ifndef TATOOINE_GL_IMGUI_H
#define TATOOINE_GL_IMGUI_H
//==============================================================================
...
DLL_API auto InputDouble3(const char* label, double v[3],
                          const char*         format = "%.3lf",
                          ImGuiInputTextFlags flags  = 0) -> bool;
...
DLL_API auto DragDouble(const char* label, double* v, double v_speed = 1.0,
    ...
    -> bool;
DLL_API auto DragDouble2(const char* label, double v[2], double v_speed = 1.0,
                         double v_min = 0.0, double v_max = 0.0,
                         const char* format = "%.3lf", float power = 1.0)
    -> bool;
DLL_API auto DragDouble3(const char* label, double v[3], double v_speed = 1.0,
                         double v_min = 0.0, double v_max = 0.0,
                         const char* format = "%.3lf", float power = 1.0)
    -> bool;
DLL_API auto DragDouble4(const char* label, double v[4], double v_speed = 1.0,
                         double v_min = 0.0, double v_max = 0.0,
                         const char* format = "%.3lf", float power = 1.0)
    -> bool;
DLL_API auto BufferingBar(const char* label, float value,
                          const ImVec2& size_arg, const ImU32& bg_col,
                          const ImU32& fg_col) -> bool;
DLL_API auto Spinner(const char* label, float radius, int thickness,
...
//                       // FLT_MAX,FLT_MAX,-FLT_MAX,-FLT_MAX aren't enough
//                       here?
//     SpringWeight       = 1.0f;
//     SpringSpacing      = -1.0f;
//     SpringSize         = 0.0f;
...
//   ImVec2 CurrentSize;    // Bounds of layout known at the beginning the
//   frame. ImVec2 MinimumSize;    // Minimum possible size when springs are
//   collapsed. ImVec2 MeasuredSize;   // Measured size with springs expanded.
//
...
//   ImVec2
//       StartCursorMaxPos;  // Maximum cursor position when BeginLayout is
//       called.
//
...
//                                      ImVec2 size);
// DLL_API void         BeginLayout(ImGuiID id, ImGuiLayoutType type, ImVec2
// size,
//                                  float align);
...
// DLL_API ImVec2 CalculateLayoutSize(ImGuiLayout& layout, bool
// collapse_springs); DLL_API ImGuiLayoutItem* GenerateLayoutItem(ImGuiLayout&
// layout,
//                                             ImGuiLayoutItemType type);
// DLL_API float            CalculateLayoutItemAlignmentOffset(ImGuiLayout&
// layout,
//                                                             ImGuiLayoutItem&
//                                                             item);
// DLL_API void TranslateLayoutItem(ImGuiLayoutItem& item, const ImVec2&
// offset); DLL_API void BeginLayoutItem(ImGuiLayout& layout); DLL_API void
// EndLayoutItem(ImGuiLayout& layout); DLL_API void AddLayoutSpring(ImGuiLayout&
// layout, float weight, float spacing); DLL_API void SignedIndent(float
// indent);
//
