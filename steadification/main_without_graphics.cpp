...
void calc(real_t t0, real_t btau, real_t ftau, size_t num_its, size_t seed_res,
          real_t stepsize, real_t desired_coverage, std::string seed_str) {
  using settings = settings_t<vf_t>;
  vf_t vf;
  ...
  create_directory(p);
  Steadification steadification(settings::domain, t0, btau, ftau, seed_res,
                                stepsize);
  ...
  auto [elapsed, sol] = tatooine::measure([&]() {
    return steadification.calc(vf, num_its, settings::num_edges, p,
...
void calc(int argc, char** argv) {
  ...
  calc<vf_t>(t0, btau, ftau,
             num_its, seed_res,
             stepsize, desired_coverage,
             seed_str);
}
//==============================================================================
int main(int argc, char** argv) {
  ...
  std::string vf = argv[1];
       if (vf == "dg")  { calc<doublegyre<real_t>>(argc, argv); }
  else if (vf == "fdg") { calc<fixed_time_doublegyre>(argc, argv); }
  else if (vf == "sc")  { calc<sinuscosinus<real_t>>(argc, argv); }
  else if (vf == "la")  { calc<laminar>             (argc, argv); }
  // else if (vf == "cy")  { calc<cylinder>            (argc, argv); }
  // else if (vf == "fw")  { calc<FlappingWing>        (argc, argv); }
  // else if (vf == "mg")  { calc<movinggyre<real_t>>  (argc, argv); }
  else if (vf == "rbc") { calc<rbc>                 (argc, argv); }
  else if (vf == "bou") { calc<tatooine::boussinesq>(argc, argv); }
  else if (vf == "cav") { calc<cavity>              (argc, argv); }
  else {
