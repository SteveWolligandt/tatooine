...
//------------------------------------------------------------------------------
double EggBox::s(double x, double y, double z, double /* t */) {
  // TODO vary parameters with t
  return a * sin(bx * (x + cx)) * cos(by * (y + cy)) - z;
}
//------------------------------------------------------------------------------
vec3 EggBox::g(double x, double y, double /* z */, double /* t */) {
  // TODO vary parameters with t
  return {a * bx * cos(bx * (cx + x)) * cos(by * (cy + y)),
          -a * by * sin(bx * (cx + x)) * sin(by * (cy + y)), -1};
}
//------------------------------------------------------------------------------
vec3 EggBox::v(double /* x */, double /* y */, double /* z */, double /* t */) {
  return vec3::zeros();
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
double TileBox::s(double x, double y, double z, double t) {
  x = x - floor(x - l * t) - l * t;
  if (std::sqrt(std::pow((x - 0.5), 2) + std::pow((z - 0), 2)) < 0.5 && y > 0 &&
      y < 1) {
    return -sin(k * t *
                std::pow(sin(M_PI * (0.5 * std::sqrt(4.0 * x * x - 4.0 * x + 1 +
                                                     4.0 * z * z) +
                                     0.5)),
                         2) *
...
//------------------------------------------------------------------------------
vec3 TileBox::g(double x, double y, double z, double t) {
...
//------------------------------------------------------------------------------
double Plane::s(double /*x*/, double /*y*/, double z, double /*t*/) {
  return z;
}
//------------------------------------------------------------------------------
vec3 Plane::g(double /*x*/, double /*y*/, double /*z*/, double /*t*/) {
  return vec3{0, 0, 1};
}
//------------------------------------------------------------------------------
vec3 Plane::v(double x, double /*y*/, double /*z*/, double /*t*/) {
...
//------------------------------------------------------------------------------
double InterpolatedField::s(double x, double y, double z, double t) {
  auto const [low_t, high_t, w] = interpSteps(t, _steps);
  return _base->s(x, y, z, low_t) * (1 - w) + _base->s(x, y, z, high_t) * w;
}
//------------------------------------------------------------------------------
vec3 InterpolatedField::g(double x, double y, double z, double t) {
  auto const [low_t, high_t, w] = interpSteps(t, _steps);
  return _base->g(x, y, z, low_t) * (1 - w) + _base->g(x, y, z, high_t) * w;
}
//------------------------------------------------------------------------------
vec3 InterpolatedField::v(double x, double y, double z, double t) {
  auto const [low_t, high_t, w] = interpSteps(t, _steps);
